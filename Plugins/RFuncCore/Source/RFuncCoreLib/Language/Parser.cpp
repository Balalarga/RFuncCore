...
{
	// _errors.push_back({token.line, token.column, std::format(::runtime(errFormat), fmt::arg("name", token.string), std::format_args("line", token.line), std::format_args("column", token.column))});
}
